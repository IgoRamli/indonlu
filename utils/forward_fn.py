...
###
# DistilBert Forward Function
# DistilBERT model does not have take token_type_ids as inputs as it relies on separator tokens only.
###
...
def distilbert_forward_word_classification(model, batch_data, i2w, is_test=False, device='cpu', **kwargs):
    ...
    # Forward model
    outputs = model(subword_batch, subword_to_word_indices_batch, attention_mask=mask_batch, labels=label_batch)
    loss, logits = outputs[:2]
